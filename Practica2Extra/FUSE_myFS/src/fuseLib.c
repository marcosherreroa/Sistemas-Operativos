...
static int my_open(const char *path, struct fuse_file_info *fi)
{
    ...
    //if sysinfo file
    if(strcmp(path, "/.myfs_sysinfo") == 0){
        fi->fh = -1; //le asginamos un descriptor de fichero que no esté utilizado por los inodos
        return 0;
    }
...
static int my_mknod(const char *path, mode_t mode, dev_t device)
{
    ...
    if(strcmp(path, "/.myfs_sysinfo") || findFileByName(&myFileSystem, (char *)path + 1) != -1 )
        return -EEXIST;
...
static int my_unlink (const char *path){
    ...
    NodeStruct * node = myFileSystem.nodes[idxNodoI];
    ...
    updateDirectory(&myFileSystem);
    NodeStruct freeNode;
    freeNode.freeNode = true;
    updateNode(&myFileSystem, idxNodoI, &freeNode);
 ...
 *
 * @param path file path
 * @param buf buffer where we will read data
...
static int my_read (const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi){
    ...
    if (fi->fh == -1){
        end = BLOCK_SIZE_BYTES - offset;
        if (size < end) end = size;

        memset(auxbuffer, 0 , size);

        if (totalRead < end){
            char line [BLOCK_SIZE_BYTES]; // para convertir los datos a string

            snprintf(auxbuffer, BLOCK_SIZE_BYTES, "Fecha de creación: %s", ctime(&myFileSystem.superBlock.creationTime));

            snprintf(line, BLOCK_SIZE_BYTES, "Tam Super Bloque: %lu \n", sizeof(SuperBlockStruct)),
            strncat(auxbuffer, line, BLOCK_SIZE_BYTES);

            snprintf(line, BLOCK_SIZE_BYTES, "Tam Directorio: %lu \n", sizeof(DirectoryStruct));
            strncat(auxbuffer, line, BLOCK_SIZE_BYTES);

            snprintf(line, BLOCK_SIZE_BYTES, "Tam Nodo-i: %lu \n", sizeof(NodeStruct)),
            strncat(auxbuffer, line, BLOCK_SIZE_BYTES);

            snprintf(line, BLOCK_SIZE_BYTES, "Tam Disco (bloques): %d \n", myFileSystem.superBlock.diskSizeInBlocks);
            strncat(auxbuffer, line, BLOCK_SIZE_BYTES);

            snprintf(line, BLOCK_SIZE_BYTES, "Num bloques libres: %d \n", myFileSystem.superBlock.numOfFreeBlocks);
            strncat(auxbuffer, line, BLOCK_SIZE_BYTES);

            snprintf(line, BLOCK_SIZE_BYTES, "Tamaño del bloque: %d \n", BLOCK_SIZE_BYTES);
            strncat(auxbuffer, line, BLOCK_SIZE_BYTES);

            snprintf(line, BLOCK_SIZE_BYTES, "Max tam nombre de archivo: %d \n", MAX_LEN_FILE_NAME);
            strncat(auxbuffer, line, BLOCK_SIZE_BYTES);

            snprintf(line, BLOCK_SIZE_BYTES, "Max bloques por archivo: %d \n", MAX_BLOCKS_PER_FILE);
            strncat(auxbuffer, line, BLOCK_SIZE_BYTES);

            snprintf(line, BLOCK_SIZE_BYTES, "Número de archivos: %d \n", myFileSystem.directory.numFiles);
            strncat(auxbuffer, line, BLOCK_SIZE_BYTES);
            
            for (int i = offset; i < BLOCK_SIZE_BYTES && totalRead < end; ++i){
                buf[totalRead] = auxbuffer[i];
                ++totalRead;
            }
        }
        ...
        while(totalRead < end) {
            ...
            for(i = offBlock; (i < BLOCK_SIZE_BYTES) && (totalRead < end); i++) {
                buf[totalRead] = auxbuffer[i];
                ++totalRead;          
            }
