...
int traverse_tree(char *dir, int (*f)()) {
	...
	while ((dp = readdir(dirp)) != NULL) {
		...
		if (!strcmp(".", dp->d_name)) continue; // cuando son iguales strcmp devuelve 0, que se interpreta como false en C
		...
		if ((ok != 1) && ((buf.st_mode & S_IFMT)==S_IFDIR))  {//probablemente -1
			totald++;
...
void print_report(char *dir) {
	...
	printf("Results for the directory %s.\n", dir); //no entiendo bien por que ponen el punto
	if (optiont) {
