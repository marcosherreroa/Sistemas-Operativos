...
extractTar(char tarName[])
{
	...
	int headerSpace = sizeof(int) + nFiles*sizeof(unsigned int) + nFiles;
		for(int i = 0; i < nFiles; ++i){
			headerSpace += strlen(header[i].name);
		}
	
	int tamAcum = headerSpace;

	for (int i = 0; i < nFiles; ++i){
		printf ("[%d]: Extrayendo %s, tam %u Bytes\n", i, header[i].name, header[i].size);
		...
		if(pid == 0){
			...
			else if (copynFile(origin, destination, header[i].size) == -1){
		...
		tamAcum += header[i].size;
	}

	while (ret == EXIT_SUCCESS && wait(&ret) > 0);
